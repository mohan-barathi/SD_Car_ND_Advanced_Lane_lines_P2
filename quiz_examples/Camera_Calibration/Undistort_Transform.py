...
def corners_unwarp(img, nx, ny, mtx, dist):
    ...
    if(ret):
            # a) draw corners
        plotted_img = cv2.drawChessboardCorners(gray_image, (8,6), corners, ret)
        #print (len(corners), corners)
        src = np.float32([corners[0],corners[1],corners[8],corners[9]])
        det = np.float32([[100,100],[250,100],[100,250],[250,250]])
...
def corners_unwarp(img, nx, ny, mtx, dist):
    ...
    # 1) Undistort using mtx and dist
    undistorted_image = cv2.undistort(img, mtx, dist, None, mtx)
    img_size = (undistorted_image.shape[1],undistorted_image.shape[1])
    # 2) Convert to grayscale
    gray_image = cv2.cvtColor(undistorted_image,cv2.COLOR_BGR2GRAY)
    # 3) Find the chessboard corners
    ret, corners = cv2.findChessboardCorners(gray_image, (8,6), None)
    ...
    if(ret):
        ...
        print (len(corners), corners)
        src = np.float32([corners[0],corners[1],corners[8],corners[9]])
        det = np.float32([[435,115],[575,115],[435,255],[575,255]])
		#det = np.float32([[100,100],[300,100],[100,300],[300,300]])
        M = cv2.getPerspectiveTransform(src,det)
        warped = cv2.warpPerspective(undistorted_image, M, img_size, flags=cv2.INTER_LINEAR)
